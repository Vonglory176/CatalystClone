...
exports.handler = async function (event, context) {
    ...
    try { //ADD BETTER ERROR MESSAGES
        ...
        if (data.type === 'checkout.session.completed') {
            ...
            const orderCount = await getOrderCount(db)
            const orderNumber = orderCount.toString().padStart(6, '0') // Format as a six-digit number
            console.log("HERE 4")
            console.log(orderNumber)

            //Saving the primary order details in Firebase
            const usersRef = db.ref(`orders`)
            await usersRef.child(`${orderNumber}`).set({
                ...data.data.object
            })
